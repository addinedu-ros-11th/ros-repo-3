...
# =====================================================================
POINTS = {
    "p0": {"x": 0.391, "y": 0.351}, "p1": {"x": 0.391, "y": 0.845}, "p2": {"x": 0.396, "y": 1.846},
    "p3": {"x": 1.200, "y": 1.746}, "p4": {"x": 1.199, "y": 0.850}, "p5": {"x": 1.650, "y": 0.850},
    "p6": {"x": 2.074, "y": 0.856}, "p7": {"x": 1.243, "y": 1.400}, "p8": {"x": 2.100, "y": 0.247},
    "p9": {"x": 2.397, "y": 0.254}, "p10": {"x": 2.388, "y": 1.297}, "p11": {"x": 1.804, "y": 1.821},
    "p12": {"x": 0.917, "y": 0.300}, "p13": {"x": 0.917, "y": 0.845}, "p14": {"x": 1.199, "y": 0.300},
    "p15": {"x": 2.000, "y": 1.400}, "p16": {"x": 1.800, "y": 0.300},"charger1": {"x": 0.250, "y": 0.650},
    "charger2": {"x": 0.450, "y": 0.650},
}

WAYPOINT_GRAPH = {
    'p0': ['p1'], 'p1': ['p0', 'p2', 'p13'], 'p2': ['p1', 'p3'],
    'p3': ['p2', 'p4', 'p7', 'p11'], 'p4': ['p3', 'p5', 'p13'],
    'p5': ['p4', 'p6'], 'p6': ['p5', 'p8', 'p15'],
    'p7': ['p3', 'p10'], 'p8': ['p6', 'p9', 'p16'],
    'p9': ['p8'], 'p10': ['p7', 'p11', 'p15'],
    'p11': ['p3', 'p10'], 'p12': ['p13', 'p14'],
    'p13': ['p1', 'p12', 'p4'], 'p14': ['p12', 'p16'],
    'p15': ['p6', 'p10'], 'p16': ['p8', 'p14']
}
...
class Nav2WaypointNavigator(Node):
        ...
        while queue:
            ...
            if node not in explored:
                for neighbour in WAYPOINT_GRAPH.get(node, []):
                    new_path = list(path); new_path.append(neighbour)
    ...
    def get_current_waypoint(self):
        ...
        try:
            trans = self.tf_buffer.lookup_transform('map', 'base_footprint', Time(), timeout=rclpy.duration.Duration(seconds=1.0))
            curr_x = trans.transform.translation.x
            curr_y = trans.transform.translation.y
            
            nearest_wp = None; min_dist = float('inf')
            for name, coord in POINTS.items():
                dist = math.hypot(coord['x'] - curr_x, coord['y'] - curr_y)
                if dist < min_dist:
                    min_dist = dist; nearest_wp = name
            return nearest_wp
    ...
    def publish_markers(self):
        ...
        for i, (name, pt) in enumerate(POINTS.items()):
            ...
            sphere.ns = "waypoints"; sphere.id = i; sphere.type = Marker.SPHERE; sphere.action = Marker.ADD
            sphere.pose.position.x = pt['x']; sphere.pose.position.y = pt['y']; sphere.pose.position.z = 0.02
            ...
            sphere.color.a = 0.8; marker_array.markers.append(sphere)

            text = Marker()
            text.header.frame_id = "map"; text.header.stamp = now
            text.ns = "labels"; text.id = i + 100; text.type = Marker.TEXT_VIEW_FACING; text.action = Marker.ADD
            text.pose.position.x = pt['x']; text.pose.position.y = pt['y']; text.pose.position.z = 0.2
            text.scale.z = 0.1; text.color.r = text.color.g = text.color.b = 0.5; text.color.a = 1.0
            text.text = name; marker_array.markers.append(text)
...
class Nav2GuiApp(QWidget):
    ...
    def init_ui(self):
        ...
        for wp_name in sorted(POINTS.keys()):
            btn = QPushButton(wp_name); btn.setFixedWidth(40)
        ...
        self.setLayout(layout); self.resize(800, 600)

    def add_destination(self, name):
        self.target_queue.append(name); self.list_widget.addItem(name)
        self.update_log(f"➕ 목적지 추가: {name}")

    def clear_targets(self):
        self.target_queue = []; self.list_widget.clear()
    ...
    def start_navigation(self):
        ...
        nav_thread = threading.Thread(target=self.ros_node.execute_navigation, args=(targets_to_run,))
        nav_thread.start()
