...
이 노드는 ROS2 환경에서 실행되며 두 가지 역할을 수행:

1. ROS2 → HTTP (상태 전파)
   - /robot{N}/odom, /robot{N}/battery 등 토픽 구독
   - malle_service PATCH /api/v1/robots/{id}/state 로 HTTP 전송

2. HTTP → ROS2 (명령 수신)
   - FastAPI 미니 서버 (port 9100)로 malle_service의 명령 수신
   - ROS2 토픽으로 퍼블리시 (cmd_vel, task_command 등)

실행:
    ros2 run malle_controller bridge_node
    또는
...
if HAS_FASTAPI:
    ...
    async def health():
        return {"status": "ok", "service": "bridge_node", "ros2": HAS_ROS2}
...
if HAS_ROS2:
    class BridgeNode(Node):
        ...
        def _push_states(self):
            """Push all robot states to malle_service via HTTP PATCH."""
            for robot_id, state in self._robot_states.items():
                motion = "MOVING" if state["speed_mps"] > 0.01 else "STOPPED"
                payload = {
                    "x_m": state["x_m"],
                    "y_m": state["y_m"],
