...
# 사용자 정의 메시지 임포트
from malle_controller.msg import TaskCommand
...
class PinkyParkingNode(Node):
    ...
    def __init__(self):
        ...
        # 제어 파라미터 및 상태
        self.target_id = None  # 명령을 받기 전까지 대기
    ...
    def _on_command_received(self, msg: TaskCommand):
        """
        robot_id에 따른 태그 번호 자동 매핑 로직
        """
        if msg.task_type == 'PARKING':
            rid = msg.robot_id
            
            # 매핑 로직: malle_01 -> 11, malle_02 -> 12, malle_03 -> 13
            if rid == 'malle_01':
                self.target_id = 11
            elif rid == 'malle_02':
                self.target_id = 12
                ...
                self.get_logger().warn(f"Unknown robot_id '{rid}'. Defaulting to Tag 11.")
                self.target_id = 11
            ...
            self.get_logger().info(f">>> {rid} MISSION START: Target Tag {self.target_id} <<<")

    def _control_loop(self):
        global latest_gray_frame
        # IDLE 상태거나 타겟 태그가 지정되지 않았으면 동작 안 함
        if self.state in ["IDLE", "DONE"] or latest_gray_frame is None or self.target_id is None:
            return
        ...
        if not target:
            if self.state != "UTURN" and time.time() - self.last_target_time > 0.3:
                self._send_twist(0.0, 0.5) # 탐색 회전
            return
    ...
    def _perform_uturn(self):
        ...
        self.state = "DONE"
        self.target_id = None # 미션 종료 후 타겟 초기화
