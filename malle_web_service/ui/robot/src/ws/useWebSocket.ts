...
export function useWebSocket({
  ...
  const connect = useCallback(() => {
    ...
    ws.onopen = () => {
      if (WS_DEBUG) console.log("[WS][robot] connected:", `${WS_BASE}${path}`);
    };

    ws.onmessage = (event) => {
      try {
        const msg: WsMessage = JSON.parse(event.data);
        if (WS_DEBUG) console.log("[WS][robot] recv:", msg.type, msg.payload);
    ...
    ws.onclose = () => {
      if (WS_DEBUG) console.log("[WS][robot] closed:", `${WS_BASE}${path}`);
      if (reconnect) {
